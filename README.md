...
| Path                                            | OS                     |
|-------------------------------------------------|------------------------|
|/usr/lib/ntfs-3g/ntfs-plugin-80000013.so         | Linux 32-bit       |
|/usr/lib64/ntfs-3g/ntfs-plugin-80000013.so       | Linux 64-bit       |
|/usr/lib/ntfs-3g/ntfs-plugin-80000013.so         | OpenIndiana 32-bit |
|/usr/lib/amd64/ntfs-3g/ntfs-plugin-80000013.so   | OpenIndiana 64-bit |
...
For debugging (e.g viewing debug messages), you can follow the following procedure

```shell
# build the library
$ make 
# link the plugin to the build dir
# (ntfs-3g only loads plugins from its installation prefix)
$ sudo mkdir /usr/lib/x86_64-linux-gnu/ntfs-3g/
$ sudo ln -sf $PWD/.libs/ntfs-plugin-80000013.so /usr/lib/x86_64-linux-gnu/ntfs-3g/ntfs-plugin-80000013.so
# mount the volume in foreground+ro mode
$ sudo mkdir /mnt/ntfs
$ sudo ntfs-3g -o no_detach,ro,allow_other,plugin=ntfs-plugin-80000013.so /dev/sdXX /mnt/ntfs
```

The plugin will be loaded as soon as a duplicate file is accessed (or a directory that contains one).

You should then see the following log message:

```
Deduplication plugin 1.2.5 for ntfs-3g
```